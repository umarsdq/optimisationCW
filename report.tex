...
\subsubsection*{Additional Analysis (as required by instructions)}
...
\textbf{Proof of smaller norm for regularised solution:}

\textit{[Your proof here. Show that any $\mathbf{u}^*$ solving the regularised problem satisfies $\|\mathbf{u}^*\|_2 \leq \|\mathbf{u}\|_2$ for any $\mathbf{u}$ solving the unregularised problem.]}
...
\textit{[Explain in your own words here. As a regulariser, $L_\epsilon$ promotes sparsity in the control signal (similar to L1 regularisation) but is smooth everywhere, making it suitable for gradient-based optimization methods.]}
...
\textbf{Gradient Descent Settings:}

\textit{[Describe your settings here. e.g., Learning rate = $10^{-4}$, Number of iterations = 10000.]}
...
\textit{[Your observations here. Compare the three cases.]}
\begin{itemize}
