...
Figure~\ref{fig:mse_vs_size_comparison} compares the MSE against the number of training points for both methods. The model that includes gradient information consistently achieves a lower validation error, indicating that it generalises better.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{plots/mse_vs_training_size_comparison.png}
\caption{MSE on validation data vs. number of training points (fixed $n=9$), with and without gradient information}
\label{fig:mse_vs_size_comparison}
\end{figure}

Despite the improved MSE, the final learned function shown in Figure \ref{fig:learned_function} is visually identical to the one learned without gradient information, suggesting both models successfully capture the function's overall shape. Incorporating the gradient information does not lead to a significant improvement in the final approximation of the function.
...
Since $\mathbf{u}$ minimises $\|S\mathbf{u}+\mathbf{c}\|^2$, $\|S\mathbf{u}+\mathbf{c}\|^2-\|S\mathbf{u}^*+\mathbf{c}\|^2\le 0$. Therefore
...
\end{align*}
By varying $\gamma$ from $10^{-5}$ to $10^5$ (with the exponent increasing in steps of $0.1$), we compute the corresponding optimal solutions and evaluate both functionals. The behaviour of each objective function with respect to $\gamma$, and the plot of $(\mathcal{J}_2(\gamma), \mathcal{J}_1(\gamma))$ (Pareto front) are shown in Figure~\ref{fig:pareto_all}.

\textbf{Impact of $\gamma$ on the total cost:}

The parameter $\gamma$ balances the two competing goals: keeping the system's internal state stable, measured by cost $\mathcal{J}_1$, and sticking to an ideal control strategy, measured by cost $\mathcal{J}_2$. 

The plots show a direct trade-off:
\vspace{-1em}
\begin{itemize}
\item{When $\gamma$ is small, the priority is keeping the system stable, which makes $\mathcal{J}_1$ low but requires deviating from the ideal control plan, making $\mathcal{J}_2$ high.}
\item{Conversely, when $\gamma$ is large, the priority shifts to following the ideal control plan almost perfectly, which makes $\mathcal{J}_2$ low at the expense of system stability, causing $\mathcal{J}_1$ to be high.}
\end{itemize}
\vspace{-1em}
The Pareto front shows the fundamental trade-off: we cannot simultaneously minimize both state deviations and control coordination. The selection of $\gamma$ is therefore a critical design choice that determines the desired balance between these competing objectives.                                                            
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{plots/pareto_all.png}
\caption{$\mathcal{J}_1$, $\mathcal{J}_2$ and the Pareto front for the multi-objective optimisation problem.}
...
The regulariser $L_\epsilon(u_i)$ is a smooth function that approximates the absolute value function $|u_i|$, defined as:
\[
L_\epsilon(u_i) = 
\begin{cases} 
\frac{1}{2}u_i^2 & \text{if } |u_i| \leq \epsilon, \\
\epsilon(|u_i| - \frac{1}{2}\epsilon) & \text{if } |u_i| > \epsilon.
\end{cases}
\]
It is defined as quadratic within the interval $[-\epsilon, \epsilon]$ and linear outside of it. This behaviour is visualized in the left plot of Figure \ref{fig:l_epsilon_explanation}, which compares $L_\epsilon(u_i)$ for two different values of $\epsilon$ against the true L1 norm (the dashed line $|u_i|$). As $\epsilon$ decreases, the approximation becomes closer to the true L1 norm.

The key advantage of this formulation is that its gradient is continuous, as shown in the right plot of Figure \ref{fig:l_epsilon_explanation}. The gradient is given by:
\[
\frac{dL_\epsilon}{du_i}(u_i) =
\begin{cases}
u_i & \text{if } |u_i| \le \epsilon, \\
\epsilon\,\mathrm{sign}(u_i) & \text{if } |u_i| > \epsilon.
\end{cases}
\]
The gradient is linear within the smooth region and constant outside, providing a smooth approximation to the sign function, which is the subgradient of the L1 norm. This differentiability is what allows us to use gradient descent to solve the optimisation problem.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{plots/l_epsilon_regulariser.png}
    \caption{A visualization of the smooth regulariser $L_\epsilon(z)$ (left) and its gradient (right), compared to the L1 norm $|z|$ and its subgradient for $\epsilon=1.0$ and $\epsilon=0.1$.}
    \label{fig:l_epsilon_explanation}
\end{figure}
...
\end{figure}
\vspace{-1em}
\textbf{Observations:} (need to go over case iii part)
\begin{itemize}
    \item \textbf{Case i ($\gamma_1=0, \gamma_2=1, \epsilon=1$):} Here, the objective function reduces to the pure $\ell_2$ regularization problem from Part II.b, and the plots are visually identical. This shows that the gradient descent implementation, given sufficient iterations, is successfully converging to the true analytical solution. The plot shows that both controls u and v are smooth and follow each other closely, but are not identical, effectively driving both state trajectories to zero.

    
    \item \textbf{Case ii ($\gamma_1=1, \gamma_2=0, \epsilon=1$):} In this case a smoothed $\ell_1$-style regularizer is used. However, with a large smoothing parameter ($\epsilon=1$), the penalty function $L_\epsilon$ has a wide quadratic region, making it behave very similarly to an $\ell_2$ norm. As a result, the control signals are almost indistinguishable from those in Case (i), pulling the controls together but not enforcing true sparsity. Similarly to Case (i), the state trajectories exhibit a smooth decay towards zero.

    \item \textbf{Case iii ($\gamma_1=1, \gamma_2=0, \epsilon=0.1$):} Reducing the smoothing parameter to $\epsilon=0.1$ makes the regularizer approximate the true $\ell_1$ norm more closely, which promotes sparsity in the difference $\mathbf{u}-\mathbf{v}$. This has a dramatic effect on the solution. The optimizer finds that the most efficient way to achieve the objective is to apply a single, large control impulse with $\mathbf{u}$ at the first time step, while keeping $\mathbf{v}$ close to zero for all time. After the initial impulse, both controls remain near zero. This strategy makes the difference $\mathbf{u}-\mathbf{v}$ sparse (non-zero only at the start) while still successfully driving both states to zero. The state trajectories for this case are notably different in their initial behaviour compared to the other cases. They exhibit a sharper, more immediate decrease in value, reflecting the highly impulsive control strategy. Despite this initial difference, the objective of driving the states to zero is still effectively achieved.
\end{itemize}
\vspace{-1em}
\end{document}